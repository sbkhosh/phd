...
!
! Code modified () by Sohrab Khosh Aghdam
! on 27/05/2015
...
SUBROUTINE perfout(str)
  ...
  if (trim(str).eq.'Incompact3d') then
     call perfout_all
  else
  ...
  99 format(i4,x,a20,x,f16.5,x,i10)
...
SUBROUTINE perfout_all
  ...
  ! Print out all the timing results to file
 
  call get_unit_nr(fperf_out)
  open(fperf_out,file=trim(outdir)//'/perf_sum.txt',&
       action='write',status='unknown')
  write (fperf_out,'(A)') ''
  write (fperf_out,'(A)') '**************************************************************'
  write (fperf_out,'(A)') '***********           performance results          ***********'
  write (fperf_out,'(A)') '**************************************************************'

  write (fperf_out,'(A)') '-------------------------------------------------------'
  write (fperf_out,'(A)') 'Timer name             #calls     Time(s)   Incl.(%)   '
  write (fperf_out,'(A)') '-------------------------------------------------------'
  do i = 1, max_timer_id
      write(fperf_out,99) timer_name(i),call_count(i),timer_value(i),100.*timer_value(i)/timer_value(1)
  end do 
  write (fperf_out,'(A)') ''

  99 format(a16,x,i9,x,f12.3,7x,f5.1)
